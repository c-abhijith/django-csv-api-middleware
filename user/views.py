...
class UserViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        try:
            ...
            #         valid_users.append(serializer.validated_data)
            #     else:
            #         invalid_rows.append({
            #             "row_data": data,
            ...
            #         already_existing_emails.append(user)
            #     else:
            #         final_valid_users.append(user)

            # response_data["success_data"] = len(final_valid_users)
            # response_data["failure_data"] = response_data["total_records"] - len(final_valid_users)
            # response_data["Detailed_validation"]["invalid_rows"] = invalid_rows
            # response_data["Detailed_validation"]["already_existing_emails_in_db"] = already_existing_emails
            # try:
            #     with transaction.atomic():
            #         User.objects.bulk_create([User(**user) for user in final_valid_users])
            # except Exception as e:
            #     return Response({
            #         "error": "Failed to save valid users to DB.",
            #         "details": str(e)
            ...
            # print("-----------------yu----------------")
            df = df.drop(nan_rows.index, errors='ignore')
