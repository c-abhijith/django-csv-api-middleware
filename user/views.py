...
class UserViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        text_response_data={}
        text_response_data["Detailed_validation"] = {}
        
        for i in range(len(df)):
            serializer =UserSerializer(data=df.iloc[i].to_dict())
            if serializer.is_valid():
                pass
                # print(serializer.data)
            else:
                for j in ["name","age","email"]:
                    
                    if j in serializer.errors:
                        if j in text_response_data["Detailed_validation"]:
                            text_response_data["Detailed_validation"][j].append(df.iloc[i].to_dict())
                            
                        else:
                            text_response_data["Detailed_validation"][j]=[df.iloc[i].to_dict()]
        ...
        return Response(text_response_data, status=status.HTTP_200_OK)
